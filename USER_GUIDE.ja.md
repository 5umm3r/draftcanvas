...
1. 画像をクリックして選択 → 左パネルの「クロップ」をクリック
2. クロップエディタが開く。ドラッグで切り抜き範囲を調整
3. 「適用」をクリックして確定

### 画像をインポートする

ローカルの画像をキャンバスに追加できます。

- PNG・JPEG・WebP ファイルをキャンバスに **ドラッグ&ドロップ**
- またはメニューからインポートダイアログを開く

インポートした画像は生成画像と同様に扱え、編集・エクスポート・参照画像として利用できます。
...
**ヒント — 一括エクスポート:**  
複数画像を選択（マーキー選択 or ⌘クリック）して「エクスポート」をクリックすると、選択画像を ZIP アーカイブとして一括書き出しできます。
...
アプリメニュー「Draft Canvas」→「設定...」（⌘,）から開きます。

| 設定 | 説明 |
|------|------|
| 外観 | ライト / ダーク / システム |
| 言語 | 日本語 / 英語 / システム。言語変更は再起動が必要 |
| 生成指示を英語に翻訳 | プロンプトを英訳してから生成。デフォルト: オフ |
| 生成アニメーション | 6スタイル（オーロラ、グリッドウェーブ、ワイヤーフレーム、パーティクル、スキャンライン、モザイク）またはランダム |
| 完了サウンド | 生成完了時のサウンド |
| 保存先フォルダ | エクスポートのデフォルト保存先 |

### ログウィンドウ

TopBar の「Logs」をクリックすると、Codex サーバーとの通信ログを表示する別ウィンドウが開きます。生成トラブルの調査に便利です。

### 自動アップデート

Draft Canvas は Sparkle 経由で自動的にアップデートを確認します。手動確認はアプリメニュー「Draft Canvas」→「アップデートを確認...」から。
