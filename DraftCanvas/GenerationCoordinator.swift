...
enum PromptFactory {
    static func prompt(for request: GenerationRequest, jobIndex: Int, jobPrompt: String? = nil) -> String {
        // バリエーション生成ではジョブごとに異なる変奏プロンプトを持つ。jobPrompt が
        // 非空ならそれを完全な指示として最優先し、request.prompt / normalizedBrief は使わない。
...
enum PromptVariator {
    ...
    /// assistantText から変奏プロンプトの配列を取り出す。コードフェンス除去後に JSON 配列としてパースする。
