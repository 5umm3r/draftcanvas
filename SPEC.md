...
- **プロンプト**: 自由テキスト（日本語・英語どちらも対応。言語は維持される）
- **count**: 生成枚数。UI 上は 1〜4 枚から選択
...
TopBar のアカウントポップオーバー（`AccountPopover`）に以下が表示されます:
...
| メソッド | 方向 | 用途 |
|---------|------|------|
| `initialize` | → | 起動・ネゴシエーション（clientInfo + experimentalApi: true） |
| `startThread` | → | 新しい会話スレッドを開始。model と reasoningEffort を渡す。threadID を返す |
| `runTurn` | → | threadID・プロンプト・参照画像パスを渡して画像生成を実行 |
| `models/list` | → | 利用可能モデル一覧を取得 |
| `account/usage` | → | アカウント使用量・プラン情報を取得 |
...
ヘルパー関数 `L(_ key: String.LocalizationValue) -> String` を通じてすべての UI 文字列を取得します。
