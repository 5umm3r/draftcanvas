...
- 語尾が「！」ならすぐ行動
- 語尾が「？」なら質問（アクションしない）
- 語尾が「！？」ならAskUserQuestionTool を使い、技術実装・UI / UX・懸念点・トレードオフなど、あらゆる観点についてユーザーに対して詳細なヒアリングを行ってください。質問は表面的・自明なものを避け、ユーザー自身もまだ言語化していない前提・判断基準・制約・優先順位が浮かび上がるような、深掘りの質問にしてください。ヒアリングは一度で終わらせず、理解が十分に完成するまで継続的にインタビューを続けてください。すべての前提・要件・意思決定の背景が明確になった段階でプランを作成してください。
...
## 注意事項
- 勝手にenvファイルを作成、編集、削除しないこと

## テキスト
- 翻訳はxcstringsを使用してアプリとして自然な文言にすること

## デザイン
- 絵文字は使用せず、アイコンやシンボルを適切に使用すること
- いわゆるAIが作ったとひと目でわかるような特徴的なデザイン（例：紫グラデーションなど）は使用しないこと

## サブエージェントのルール
- 調査・探索・並列分析はサブエージェントへオフロードしてメインコンテキストを保護する
- サブエージェント1つにつき1タスクに絞る
- Agent tool 呼び出し時は必ず `model: "sonnet"` を指定する、ただし高度な推論が必要なタスクのみOpusを継承する
- 調査の際は必ず一調査一報告を行い、課題解決できない場合のみ次の調査をしていいか確認すること
