...
final class ImageVectorizerTests: XCTestCase {
    ...
    func testProcess_solidColorPNG_producesSVGWithValidHeader() async throws {
        ...
        do {
            ...
            XCTAssertTrue(svgString.hasPrefix("<?xml"), "SVG must start with XML declaration")
            XCTAssertTrue(svgString.contains("<svg"), "SVG must contain svg element")
            XCTAssertTrue(svgString.contains("</svg>"), "SVG must close svg element")
        } catch ImageVectorizationError.noContoursFound {
            // Solid single-color may not produce contours — acceptable
        } catch ImageVectorizationError.contourDetectionFailed {
            // Vision runtime error in test environment — acceptable
        }
    }
    ...
    func testProcess_solidColorPNG_previewIsValidPNG() async throws {
        ...
        do {
            ...
            XCTAssertEqual(Array(result.previewPNGData.prefix(8)), pngHeader, "Preview must be PNG format")
        } catch ImageVectorizationError.noContoursFound {
            // acceptable for solid image
        } catch ImageVectorizationError.contourDetectionFailed {
            // acceptable in test env
        }
    }

    func testVectorizationResult_svgContainsPathElement() async throws {
        // Two-color image: red left half, blue right half
        let png = makeTwoColorPNG(width: 64, height: 64)
        do {
            let result = try await ImageVectorizer.process(data: png)
            let svg = String(data: result.svgData, encoding: .utf8) ?? ""
            XCTAssertTrue(svg.contains("<path"), "SVG should contain at least one path element")
        } catch ImageVectorizationError.noContoursFound {
            // acceptable
        } catch ImageVectorizationError.contourDetectionFailed {
            // acceptable in test env
