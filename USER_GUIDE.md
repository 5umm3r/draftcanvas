...
| マスクして編集 | ブラシで範囲を指定して、その部分だけ AI に修正させる |
| マスクして除去 | ブラシで塗った箇所のオブジェクトを背景で自然に埋める |
...
| エクスポート | 保存形式を選んで書き出す |
| Finderで表示 | 保存ファイルを Finder で開く |
| 削除 | キャンバスから削除 |

### マスクして編集 / マスクして除去（インペイント）

特定の部分だけを変えたいときに使います。

1. 画像をクリックして選択 → 左パネルの「マスクして編集」または「マスクして除去」をクリック
2. ブラシで変えたい箇所を塗る
3. モードに応じて操作:
   - **マスクして編集**: プロンプトに指示を入力して「生成」をクリック（塗った範囲を AI が書き換える）
   - **マスクして除去**: 「生成」をクリックするだけ（塗った範囲のオブジェクトを背景で自然に埋める）
...
画面右上のアカウントアイコンから、Codex の使用量（5時間ウィンドウ / 週次）を確認できます。

---

## よくある質問

**Q. Codex CLI が見つからないと言われる**  
A. `which codex` で場所を確認し、アプリの設定からパスを手動指定してください。
...
ライセンスキーは「アプリメニュー → ライセンスを有効化…」から入力できます。
