...
テキストを打ち込むだけで画像が生成されます。生成した画像はそのままアプリ内で編集でき、背景の除去・高解像度化・SVG 変換まで一気通貫でこなせます。サブスクリプションなし、¥1,980 の買い切り。
...
- **トライアル期間中**: アプリメニューバー（Draft Canvas メニュー）に「トライアル: 残 N 日」ボタンが表示されます（残 3 日以下は警告アイコン付き）。クリックでライセンス入力シートを開けます。
- **トライアル期限切れ**: 起動時に自動でライセンス入力シートが表示されます。メニューの「トライアル期限切れ」からも開けます。
- **ライセンス認証済み**: これらのメニュー項目は表示されません。

OSS ライセンスの確認: アプリメニュー「Draft Canvas → About Draft Canvas」のクレジット欄で確認できます。
