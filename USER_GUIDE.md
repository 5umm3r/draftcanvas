...
**ヒント — 生成指示を英語に翻訳:**  
設定の「生成指示を英語に翻訳」をオンにすると、入力プロンプトを英訳してから生成します。結果のブレが減る場合がありますが、トークン消費が増えます。デフォルトはオフです。
...
TopBar に 5時間・週次の使用量ピルが常時表示されます。各ピルには識別子（「5h」/「weekly」）・⚡・残量パーセント・残量バー・「あと何日何時間」のリセット目安が表示されます。

アカウントアイコンをクリックするとポップオーバーが開き、アカウント種別・メール・プラン・Codex バージョンを確認できます。未ログイン時は `codex login` のガイダンスが、ChatGPT Free プランでは画像生成不可の警告が表示されます。
...
- **トライアル期間中**: アプリメニューバー（DraftCanvas メニュー）に「トライアル: 残 N 日」ボタンが表示されます（残 3 日以下は警告アイコン付き）。クリックでライセンス入力シートを開けます。
- **トライアル期限切れ**: 起動時に自動でライセンス入力シートが表示されます。メニューの「トライアル期限切れ」からも開けます。
- **ライセンス認証済み**: これらのメニュー項目は表示されません。

OSS ライセンスの確認: アプリメニュー「DraftCanvas → About DraftCanvas」のクレジット欄で確認できます（vtracer / visioncortex / image / oxipng / pngquant）。
