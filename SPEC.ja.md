...
2. [ユーザー向け機能仕様](#2-ユーザー向け機能仕様)
   ...
   - 2.13 [生成中プレースホルダアニメーション](#213-生成中プレースホルダアニメーション)
...
Draft Canvas は macOS 向けの AI 画像生成・編集デスクトップアプリケーションです。テキストプロンプトから画像を生成し、インペイント・背景除去・アップスケール・ベクター化といった後処理をプロジェクト単位のキャンバスで管理します。画像生成には Codex CLI を `app-server` モードでサブプロセス起動し、JSON-RPC over stdio 経由で OpenAI の画像生成 API（gpt-image pre-2.0）を呼び出します。すべての処理は Mac 上で完結します。画像生成には ChatGPT Plus 以上のサブスクリプションが必要です。
...
| 選択解除 | キャンバス余白をクリック |
| クリップボードにコピー | ⌘C で選択中の画像をシステムクリップボードにコピー（`ImageClipboard`） |
...
**スケッチエディタ仕様（`SketchEditorSheet.swift`）:**
...
| 価格 | 無料 |
| 商用利用 | 可 |
| スポンサー | [GitHub Sponsors](https://github.com/sponsors/5umm3r) / [Polar](https://buy.polar.sh/polar_cl_fZgIt4TMaHLTt1ah2aYDWoI9BGoY2KEuTx5fC0G2ehc) |
...
モーダルシート: `BackgroundRemovalPreviewSheet` / `UpscalePreviewSheet` / `VectorizingOverlay` / `MaterialExtractionSheet` / `InpaintingMaskEditor` / `OutpaintEditorSheet` / `CropEditorSheet` / `ExpandedImageSheet` / `ExportOptionsSheet` / `FilteringProjectCreationSheet`
...
| クロップエディタ | `Editors/Crop/CropEditorSheet.swift`, `CropCanvasNSView.swift`, `ViewModel/DraftCanvasViewModel+Crop.swift` |
| Sketch エディタ | `Editors/Sketch/SketchEditorSheet.swift`, `SketchCompositor.swift`, `SketchModels.swift`, `ViewModel/DraftCanvasViewModel+Sketch.swift` |
...
| 画像クリップボード | `ImageClipboard.swift` |
| プロンプトバリエーション | `PromptVariator.swift` |
| リリースノート | `Views/ReleaseNotes/ReleaseNotesView.swift`, `ReleaseNotesWindowController.swift`, `ReleaseNotesLoader.swift` |
| マテリアル抽出 | `MaterialExtraction/MaterialExtractor.swift`, `MaterialExtraction/Pipeline/` |

---

## 6. ドメインモデル

ソース: `DraftCanvas/Models/`（型ごとに個別ファイル）
...
struct GenerationRequest: Equatable {
    var prompt: String
    var count: Int               // UI 上限: 8。normalizedCount で 1〜24 にクランプ（サーバー側バリデーション）
...
| マテリアル抽出 | `DraftCanvas/MaterialExtraction/` |
